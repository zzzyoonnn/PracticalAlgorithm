...
출력
- 첫째 줄에 주어진 탑들의 순서대로 각각의 탑들에서 
  발사한 레이저 신호를 수신한 탑들의 번호를 하나의 빈칸을 사이에 두고 출력한다. 
...
public class Baekjoon2493 {
  public static void main(String[] agrs) throws IOException {
    ...
    int[] answer = new int[n];      // 각 탑의 수신 탑을 저장할 배열

    for (int i = 0; i < n; i++) {
      // 스택이 비어있지 않고, 현재 탑의 높이가 스택의 맨 위 탑보다 높을 때까지 반복
      while (!stack.isEmpty() && heights[i] > heights[stack.peek()]) stack.pop();

      // 스택이 비어있지 않다면, 현재 탑의 수신 탑은 스택의 맨 위 탑
      if (!stack.isEmpty()) answer[i] = stack.peek() + 1;

      stack.push(i);    // 현재 탑을 스택에 추가
    }
    for (int i : answer) System.out.print(answer[i] + " ");
  }
}
